...
def train(args):
    ...
    if args.vllm_num_engines is not None and args.vllm_num_engines > 0:
        ...
        if args.use_autorule:
            judge_vllm_engines = create_vllm_engines(
                ...
                args.actor_num_nodes * args.actor_num_gpus_per_node // args.ring_attn_size,
                pg if args.colocate_all_models else None,
                args.judge_vllm_gpu_memory_utilization,
                args.judge_vllm_enable_sleep,
            )
        else:
    ...
    refs = actor_model.async_fit_actor_model(
        ...
        args.remote_rm_url,
        reward_fn=reward_fn,
        use_autorule=args.use_autorule,
        use_length_norm=args.use_length_norm,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use the AutoRule method",
    )
    parser.add_argument(
        "--use_length_norm",
        action="store_true",
        default=False,
        help="Use length normalization",
    )
    parser.add_argument(
        "--rules_version",
        type=str,
        default=None,
        help="Version of rules to use (uf, mt, uf_exp)",
    )
    parser.add_argument(
        "--judge_vllm_num_engines", type=int, default=None, help="number of judge vLLM Engines for AutoRule"
    )
    parser.add_argument(
        "--judge_vllm_tensor_parallel_size",
        type=int,
        default=1,
        help="tensor parallel size of judge vLLM Engine for multi-GPU inference",
    )
    parser.add_argument(
        "--judge_vllm_enable_sleep",
        action="store_true",
        default=False,
        help="Enable sleep mode for vLLM when using --colocate_all_models",
    )
    ...
    parser.add_argument("--judge_pretrain", type=str, default=None, help="HF model name or path")
