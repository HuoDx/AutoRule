...
class BaseExperienceMaker(ABC):
    ...
    def __init__(
        ...
        self.judge_tokenizer = get_tokenizer(
            "meta-llama/Meta-Llama-3-8B-Instruct", None, "left", strategy, use_fast=not strategy.args.disable_fast_tokenizer
        )
...
class RemoteExperienceMaker(BaseExperienceMaker):
    def __init__(self, *args, vllm_engines: List = None, judge_vllm_engines: List = None, packing_samples=False, **kwargs):
        super().__init__(*args, **kwargs)
        self.vllm_engines = vllm_engines
        self.judge_vllm_engines = judge_vllm_engines
    ...
    def make_experience(self, samples_list: List[Samples]) -> List[Experience]:
        ...
        if self.strategy.ring_attn_group is None or self.strategy.ring_attn_rank == 0:
            ...
            for i, (seq, packed_lens) in enumerate(zip(sequences_cpu_list, packed_seq_lens_list)):
                if not self.packing_samples:
                    queries = self.tokenizer.batch_decode(seq, skip_special_tokens=True)
                else:
                    sequences_list = []
                    offset = 0
            ...
            print("number of samples:", len(queries_list))
        ...
        # rule_rewards_list, full_rule_dict = self._generate_judge_vllm(queries_list)
        # rule_rewards_list = rule_rewards_list.chunk(len(samples_list))
        # for k, v in full_rule_dict.items():
            # full_rule_dict[k] = v.chunk(len(samples_list))
        # rule_dicts = []
        # for i in range(len(rule_rewards_list)):
            # rule_dicts.append({k: v[i] for k, v in full_rule_dict.items()})
        model_rewards_list = ref_values[2]
        # import matplotlib.pyplot as plt

        # # flatten per‐sample response lengths and average model rewards
        # response_lengths = []
        # model_reward_means = []
        # for samples, mr in zip(samples_list, model_rewards_list):
        #     # samples.response_length is a 1D tensor of shape (micro_batch_size,)
        #     lengths = samples.response_length.cpu().tolist()
        #     # mr is a 1D tensor of shape (micro_batch_size,)
        #     means = mr.cpu().tolist()
        #     response_lengths.extend(int(l) for l in lengths)
        #     model_reward_means.extend(float(m) for m in means)

        # # plot
        # # prepare data
        # x = np.array(response_lengths)
        # y = np.array(model_reward_means)
        ...
        # # formatting
        # ax.set_title(f"llama-3-8b-grpo-33k-length-norm\n$R^2$={r2:.3f}")
        ...
        # # save plot
        # plot_dir = "/openrlhf/plots"
        # os.makedirs(plot_dir, exist_ok=True)
        # plot_path = os.path.join(plot_dir, f"scatter_response_length_vs_model_reward_{int(time.time())}.png")
        # fig.savefig(plot_path)
        # plt.close(fig)
        # print(f"Saved scatter to {plot_path}")
        rewards_list = model_rewards_list
        # rewards_list = [self.reward_fn([model_rewards_list[i], rule_rewards_list[i]]) for i in range(len(samples_list))]
        ...
        for i, (samples, action_log_probs, base_action_log_probs, value, rewards, model_rewards) in enumerate(
            zip(samples_list, action_log_probs_list, base_action_log_probs_list, value_list, rewards_list, model_rewards_list)
        ):
            ...
            # r_r = rule_rewards.to(device)
            m_r = model_rewards.to(device)
            # for k in rule_dict.keys():
                # rule_dict[k] = rule_dict[k].to(device)
    ...
    def _generate_with_hf(self, all_prompts: List[str], all_labels, **generate_kwargs) -> List[Samples]:
        ...
        for i in range(0, len(all_prompts), args.micro_rollout_batch_size):
            ...
            inputs = self.tokenize_fn(self.tokenizer, prompts, self.prompt_max_len, device="cuda")
    ...
    def _generate_vllm(self, all_prompts: List[str], all_labels, **kwargs) -> List[Samples]:
        ...
        all_prompts = sum([[prompt] * args.n_samples_per_prompt for prompt in all_prompts], [])
        all_labels = sum([[label] * args.n_samples_per_prompt for label in all_labels], [])
        all_prompt_token_ids = self.tokenize_fn(self.tokenizer, all_prompts, self.prompt_max_len, padding=False)["input_ids"]
    ...
    def _generate_judge_vllm(self, all_responses: List[str], **kwargs) -> List[Samples]:
        from vllm import SamplingParams
        ...
        # Select LLM engines: assign each rank an engine, or cycle through engines if world_size < engine_count
        if len(self.judge_vllm_engines) <= world_size:
            llms = [self.judge_vllm_engines[rank % len(self.judge_vllm_engines)]]
        else:
            llms = self.judge_vllm_engines[rank::world_size]
        ...
        for response in all_responses:
            for rule in rules:
                all_prompts.append(get_rule_prompt(response, rule))

        # Expand prompt list based on the number of samples per prompt
        # all_prompts = sum([[prompt] * args.n_samples_per_prompt for prompt in all_prompts], [])
        # all_labels = sum([[label] * args.n_samples_per_prompt for label in all_labels], [])
        all_prompts_templated = [self.judge_tokenizer.apply_chat_template(prompt, tokenize=False, add_generation_prompt=True) for prompt in all_prompts]
        all_prompt_token_ids = self.tokenize_fn(self.judge_tokenizer, all_prompts_templated, self.prompt_max_len, padding=False)["input_ids"]
        ...
        output_list = []
        for i in range(0, len(all_outputs), args.micro_rollout_batch_size):
            output_ids = [output.outputs[0].token_ids for output in all_outputs[i : i + self.strategy.args.micro_rollout_batch_size]]
            output_list.extend(self.tokenizer.batch_decode(output_ids, skip_special_tokens=True))
        
        rule_reward_dict = defaultdict(list)
        rule_rewards = []
        print("Sample output:", output_list[0])
        for i in range(0, len(output_list), len(rules)):
            for j, output in enumerate(output_list[i : i + len(rules)]):
                rule_reward_dict[f"rule_{j}"].append(1.0 if "[[Yes]]" in output else 0.0)
            rule_reward = [1.0 if "[[Yes]]" in output else 0.0 for output in output_list[i : i + len(rules)]]
            rule_rewards.append(sum(rule_reward) / len(rule_reward))

        for k, v in rule_reward_dict.items():
            rule_reward_dict[k] = torch.tensor(v)
        
        return torch.tensor(rule_rewards), rule_reward_dict
...
def get_rule_prompt(conversation_str, rule):
