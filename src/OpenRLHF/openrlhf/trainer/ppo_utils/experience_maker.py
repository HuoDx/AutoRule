...
from openrlhf.trainer.ppo_utils.rules import get_rules
...
class BaseExperienceMaker(ABC):
    ...
    def __init__(
        ...
        self.use_autorule = use_autorule
...
class RemoteExperienceMaker(BaseExperienceMaker):
    ...
    def make_experience(self, samples_list: List[Samples]) -> List[Experience]:
        ...
        if self.use_autorule:
            ...
            for _ in range(len(model_rewards_list)):
                rule_rewards_list.append(None)
                rule_dicts.append(None)
        ...
        for i, (samples, action_log_probs, base_action_log_probs, value, rewards, model_rewards, rule_rewards, rule_dict) in enumerate(
            zip(samples_list, action_log_probs_list, base_action_log_probs_list, value_list, rewards_list, model_rewards_list, rule_rewards_list, rule_dicts)
        ):
            ...
            if self.use_length_norm:
                r = r - torch.relu(0.5 * (samples.response_length / 300) - 0.5)
                m_r = m_r - torch.relu(0.5 * (samples.response_length / 300) - 0.5)
            if self.use_autorule:
    ...
    def _generate_judge_vllm(self, all_responses: List[str], **kwargs) -> List[Samples]:
        rules = get_rules(self.rules_version)
        ...
        for i in range(0, len(all_outputs), args.micro_rollout_batch_size):
            ...
            output_list.extend(self.judge_tokenizer.batch_decode(output_ids, skip_special_tokens=True))
        ...
        for i in range(0, len(output_list), len(rules)):
            ...
            rule_rewards.append(((sum(rule_reward) / len(rule_reward)) - 0.75)*10)
