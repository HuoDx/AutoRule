...
class RewardDataset(Dataset):
    ...
    def __init__(
        ...
        dataset = dataset.filter(self.filter_dataset, num_proc=num_processors)
    ...
    # https://github.com/PorUna-byte/PAR/blob/master/utils/download.py#L52
    def filter_dataset(self, data, max_filter_length=512):
        def split_prompt_and_responses(ex):
            return ex['prompt'], ex['chosen'][-1]['content'], ex['rejected'][-1]['content']
        
        chosen_score = data["score_chosen"]
        rejected_score = data["score_rejected"]

        prompt, chosen, rejected = split_prompt_and_responses(data)

        if len(self.tokenizer.encode(prompt)) < max_filter_length and len(self.tokenizer.encode(chosen)) < max_filter_length \
        and len(self.tokenizer.encode(rejected)) < max_filter_length and chosen_score>rejected_score and \
